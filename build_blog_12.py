"""
Blog #12 Builder: Email Marketing Automation for Saudi Businesses
Builds EN HTML, translates section-by-section via Gemini, builds AR HTML, creates Word docs.
"""
import sys, io, os, json, time, re, shutil
...
GOOGLE_API_KEY = "AIzaSyC2GgCgwU8af6rL0Gvgi-7G_3QdFF5-92c"
...
ASSETS_DIR = r"C:\Users\Lenovo\Documents\Ensign-Website\assets\blog"
...
EN_SECTIONS = [
    # (type, content)  type: p, h2, h3, strong_p
...
print("English content defined.")

# ─── Gemini Translation Helper ─────────────────────────────────────────
def gemini_translate(text, retries=3):
    """Translate one section to Arabic: ترجم بصيغ أكثر بلاغة ودقة"""
    prompt = f"ترجم بصيغ أكثر بلاغة ودقة\n\n{text}"
...
print("\nTranslating section by section via Gemini...")
print(f"  Translating title...")
AR_TITLE = gemini_translate(EN_TITLE)
print(f"  AR title: {AR_TITLE[:60]}")

print(f"  Translating meta description...")
AR_META = gemini_translate(EN_META)

print(f"  Translating H1...")
AR_H1 = gemini_translate(EN_H1)

AR_SECTIONS = []
for i, (typ, content) in enumerate(EN_SECTIONS):
    if content.startswith('<a '):
        # Don't translate HTML links — extract text part only
        AR_SECTIONS.append((typ, content))
        continue
    print(f"  [{i+1}/{len(EN_SECTIONS)}] Translating {typ}: {content[:50]}...")
...
for i, (q, a) in enumerate(EN_FAQ):
    print(f"  FAQ Q{i+1} question...")
    ar_q = gemini_translate(q)
    print(f"  FAQ Q{i+1} answer...")
    ar_a = gemini_translate(a)
    AR_FAQ.append((ar_q, ar_a))

print("\nTranslating related links labels...")
AR_RELATED = []
...
def build_article_html(sections, faq, related, lang='en'):
    """Build the <article> inner HTML from sections."""
    lines = []
    for typ, content in sections:
        if typ == 'p':
            ...
            # Bold first clause up to first period or colon
    ...
    h3_related = "قراءات ذات صلة" if lang == 'ar' else "Related Reading"
    lines.append(f'        <h3>{h3_related}</h3>')
    lines.append('        <ul>')
    ...
    h2_faq = "أسئلة شائعة" if lang == 'ar' else "Frequently Asked Questions"
    lines.append(f'        <h2>{h2_faq}</h2>')
    lines.append('')
    for q, a in faq:
...
def build_html(lang, title, meta, h1, article_body, pub_date, slug):
    ...
    alt_href = ar_href if lang == 'en' else en_href
    ...
    cta_title_txt = "Stop broadcasting. Start converting." if lang == 'en' else "توقف عن البث. ابدأ بالتحويل."
    cta_sub_txt = ("Your email list is either making money or costing money. The difference is how it is set up. "
                   "Book a call and find out where yours stands.") if lang == 'en' else \
    ...
    ar_link = "العربية" if lang == 'en' else None
    en_link = "English" if lang == 'ar' else None
    home_href = "../../ar/" if lang == 'ar' else "../"
    blog_href = "../../ar/blog/" if lang == 'ar' else "../blog/"

    og_locale = "ar_SA" if lang == 'ar' else "en_US"
    og_locale_alt = "en_US" if lang == 'ar' else "ar_SA"

    img_alt_en = "Email automation sequences visualised as flowing data pipelines — triggers, behavioural signals and automated send paths"
    img_alt_ar = "تصور مرئي لتسلسلات أتمتة البريد الإلكتروني كخطوط بيانات متدفقة"
    img_alt = img_alt_ar if lang == 'ar' else img_alt_en

    # nav lang switcher
    if lang == 'en':
        lang_switch = f'<a href="{ar_href}" class="nav-lang" hreflang="ar">العربية</a>'
    else:
        lang_switch = f'<a href="{en_href}" class="nav-lang" hreflang="en">English</a>'

    # footer links adjust for ar
    if lang == 'ar':
        ai_link = "/ai-solutions.html"
        agency_link = "/agency.html"
        os_link = "/ensign-os.html"
        v_link = "/vision-2030.html"
        about_link = "/ar/about.html"
        work_link = "/ar/work.html"
        careers_link = "/ar/careers.html"
        blog_link = "/ar/blog/"
    else:
        ai_link = "../ai-solutions.html"
        agency_link = "../agency.html"
        ...
        work_link = "../work.html"
        careers_link = "../careers.html"
        blog_link = "../blog/"

    logo_src = "../../assets/logos/logo-main.png" if lang == 'ar' else "../assets/logos/logo-main.png"
...
}})(window,document,'script','dataLayer','GTM-TTFT7PF7');</script>
...
<nav class="site-nav">
  <div class="nav-inner">
    ...
    <div class="nav-links">
      {lang_switch}
      <a href="https://cal.com/ensign-ai-agency-q4mmzg/30min" target="_blank" rel="noopener noreferrer" class="nav-cta" data-event-name="book_call_click" data-cta-location="nav">{"احجز مكالمة" if lang == "ar" else "Book a Call"}</a>
    </div>
  </div>
</nav>
...
<section class="section blog-post-cta">
  <div class="container">
    <div class="blog-cta-inner" data-animate="fade-up">
      ...
      <h2 class="blog-cta-title">{cta_title_txt}</h2>
      <p class="blog-cta-sub">{cta_sub_txt}</p>
  ...
  <div class="container">
    <div class="footer-top">
      <div class="footer-brand">
        ...
        <p class="footer-tagline">{"الطبقة الذكية التي تعتمدها الشركات الحديثة للنمو." if lang == "ar" else "The intelligent layer modern businesses use to grow."}</p>
      </div>
      <div class="footer-links">
        <div class="footer-col">
          <p class="footer-col-title">{"الخدمات" if lang == "ar" else "Services"}</p>
          <a href="{ai_link}">{"حلول AI" if lang == "ar" else "AI Solutions"}</a>
          <a href="{agency_link}">{"الوكالة" if lang == "ar" else "Agency"}</a>
          <a href="{os_link}">Ensign OS</a>
          <a href="{v_link}">{"رؤية 2030" if lang == "ar" else "Vision 2030"}</a>
        </div>
        <div class="footer-col">
          <p class="footer-col-title">{"الشركة" if lang == "ar" else "Company"}</p>
          <a href="{about_link}">{"من نحن" if lang == "ar" else "About"}</a>
          <a href="{work_link}">{"أعمالنا" if lang == "ar" else "Work"}</a>
          <a href="{careers_link}">{"الوظائف" if lang == "ar" else "Careers"}</a>
          <a href="{blog_link}">{"المدونة" if lang == "ar" else "Blog"}</a>
        </div>
        <div class="footer-col">
          <p class="footer-col-title">{"تواصل" if lang == "ar" else "Connect"}</p>
          ...
          <a href="mailto:contact@ensignksa.com">{"راسلنا" if lang == "ar" else "Email Us"}</a>
        </div>
      </div>
    </div>
    <div class="footer-bottom">
      <p>{"© 2026 مؤسسة إنسيان للدعاية والإعلان. جميع الحقوق محفوظة." if lang == "ar" else "© 2026 Ensign Establishment for Advertising. All rights reserved."}</p>
      <div class="footer-legal">
        <a href="/privacy-policy.html">{"سياسة الخصوصية" if lang == "ar" else "Privacy Policy"}</a>
        <a href="/terms-and-conditions.html">{"الشروط والأحكام" if lang == "ar" else "Terms"}</a>
      </div>
    </div>
  </div>
</footer>

<script src="{favicon_path}js/main.js?v=3" defer></script>
</body>
</html>"""
...
def build_docx(title, sections, faq, related, lang='en', out_path=None):
    doc = Document()
    # Title
    ...
    for typ, content in sections:
        if typ == 'h2':
            ...
            if m:
                ...
                run.bold = True
                p.add_run(content[len(m.group(1)):])
            else:
...
print("\nBuilding Word docs...")
build_docx(EN_TITLE, EN_SECTIONS, EN_FAQ, EN_RELATED, lang='en', out_path=en_docx)
build_docx(AR_TITLE, AR_SECTIONS, AR_FAQ, AR_RELATED, lang='ar', out_path=ar_docx)

print("\nAll done. Summary:")
print(f"  EN HTML:  {EN_OUT}")
print(f"  AR HTML:  {AR_OUT}")
print(f"  EN docx:  {en_docx}")
print(f"  AR docx:  {ar_docx}")
print("\nNext: generate featured image via Higgsfield, then add to blog indexes and deploy.")
